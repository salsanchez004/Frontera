...
class Box
{
    ...
    public function draw($text)
    {
        ...
        foreach ($lines as $line) {
			...
			if($n==0 && $this->lineHeight < 1) {
				$lineaNueva = strtr($line, array("Á"=>"A", "É"=>"E", "Í"=>"I", "Ó"=>"O", "Ú"=>"U", "Ü"=>"U", "Ñ"=>"N"));
//				$lineaNueva = preg_replace(array("/[Á]/i"),array("A"),$line);
//				$respMatch = preg_match('/[Á|É|Í|Ó|Ú|Ñ]+/', $line);
//				$a = fopen("baseline.txt","a");
//				fwrite($a, "Linea: ".$line."\n");
//				fwrite($a, "Linea nueva: ".$lineaNueva."\n");
//				if (strpos($line,array('Á','É','Í','Ó','Ú','Ñ')) !== false) {
//				if ($line !== $lineaNueva) {
//					fwrite($a, "Lineas diferentes\n");
					$this->baseline = -0.1;
//				}
//				else {
//					$this->baseline = $baselineO;	
//				}
//				fwrite($a, "Baseline: ".$this->baseline."\n");
//				fclose($a);
			}
//			else {
//				$this->baseline = $baselineO;
//			}
    ...
    {
		$a=fopen("tamFuente.txt","a");
		fwrite($a, "x: ".$x.", y: ".$y.", fontSize: ".$this->fontSize.", pt: ".$this->getFontSizeInPoints()."\n");
		fclose($a);
        imagefttext(
            $this->im,
