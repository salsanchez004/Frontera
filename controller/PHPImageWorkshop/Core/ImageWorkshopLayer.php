...
//			$this->layers[$layerId]->width = round($this->layers[$layerId]->width);
//			$this->layers[$layerId]->height = round($this->layers[$layerId]->height);
//			$this->layerPositions[$layerId]['x'] = round($this->layerPositions[$layerId]['x']);
//			$this->layerPositions[$layerId]['y'] = round($this->layerPositions[$layerId]['y']);
				
            $imagesToMerge[$layerLevel] = $this->layers[$layerId]->getResult();
			
//			if ($backgroundColor == "prueba")	error_log("Aqui entra ".$layerLevel.", ID: ".$layerId.", layer: ".print_r($this->layers[$layerId],true)."; layerPos: ".print_r($this->layerPositions[$layerId], true));  
...
//				if ($backgroundColor == "prueba")	error_log("Pasa merge ", 0); 
                $imagesToMerge[$layerLevel] = $virginLayoutImageTmp;
                unset($virginLayoutImageTmp);
            }
        }
		
//		if ($backgroundColor == "prueba")	 error_log("Sale del ciclo ", 0); 
        $iterator = 1;
        $mergedImage = $this->image;
        ksort($imagesToMerge);

        foreach ($imagesToMerge as $imageLevel => $image) {
//			if ($backgroundColor == "prueba")	 error_log("Alla entra ".$imageLevel, 0); 
